...
const ProjectCard = ({ index, name, description, image, extraImages = [] }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex(
        (prevIndex) => (prevIndex + 1) % (extraImages.length + 1),
      );
    }, 4000);
  ...
  const currentImage =
    currentImageIndex === 0 ? image : extraImages[currentImageIndex - 1];
